...
<div class="page-content">
	...
	<div class="card">
		...
		<?php
		$query="SELECT * FROM lecturer where id='$_GET[id]'";
		$result=mysqli_query($conn,$query);
		$row=mysqli_fetch_assoc($result);
		?>
		<form action="admin_lecturerUpdate.php" method="post" onsubmit="checkpassword(event)">
			<input type="hidden" id="oldID" name="oldID" value="<?=$row['id']?>">
			<table class="table borderless">
			<tr>
			<td>Lecturer ID</td>
			<td><input class="form-control" id="id" type="text" placeholder="Enter Lecturer ID" name="id" value="<?=$row['id']?>" required /></td>
			</tr>
			<tr>
			<td>Lecturer Name</td>
			<td><input class="form-control" id="name" type="text" placeholder="Enter Lecturer Name" name="name"  value="<?=$row['name']?>" required /></td>
			</tr>
			<tr>
			<td>Password</td>
